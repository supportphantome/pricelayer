...
**Bootstrap before the first `lingui extract-experimental` run (Step 8 in the main SKILL):** for every route that declares `import(\`./locales/{route}/${locale}.ts\`)`, create a compiled-catalog stub at each `locales/{route}/{locale}.ts` target containing a single line:
...
See the canonical writeup in the main SKILL Step 3 ("Before first extract: seed route-scoped catalog stubs") — this section is the Start-specific numbered restatement.
