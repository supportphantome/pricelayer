...
**Detection signal:** `react-router@^7` in `dependencies`, `@react-router/dev@^7` in `devDependencies`, a `react-router.config.{ts,js}` at the repo root, **and** `@vitejs/plugin-react-swc` in devDeps. If only `@vitejs/plugin-react` is present (or neither), use the Babel sibling at `../lingui.setup.md` instead.
...
**This modifies `vite.config.ts`.** Describe the changes to the user before making them: keeping `reactRouter()`, adding `@vitejs/plugin-react-swc` with `['@lingui/swc-plugin', {}]` in its `plugins` array, and adding `lingui()` as a top-level Vite plugin.
...
2. `react({ plugins: [['@lingui/swc-plugin', {}]] })` — runs the SWC macro transform on every module. The macro must expand **before** `lingui()` reads the source for catalog extraction.
...
If the project already has SWC plugins configured in the existing `react()` call, append `['@lingui/swc-plugin', {}]` to the existing `plugins` array rather than overwriting it. The tuple shape `[name, options]` (string + object) is mandatory — passing the plugin name as a bare string silently disables it. Preserve every other plugin that was already in the top-level `plugins[]` (`tsconfigPaths()`, `tailwindcss()`, custom plugins, etc.) and keep them in their original order — only insert the three plugins above at the positions described.

> **`react-router.config.ts` is unrelated.** This file lives at the repo root and configures `ssr`, `prerender`, `appDirectory`, etc. — it has no `swc` or `transform` option. Do not edit it for Lingui integration.
